...
export const useFetchChatList = (isAuthenticated) => {
    return useQuery({
        queryKey: ['chat-list'],
        queryFn: fetchChatListFn,
        enabled: Boolean(isAuthenticated),
        staleTime: 1000 * 30,
        gcTime: 1000 * 60 * 5,
        refetchOnWindowFocus: true,
        retry: 1,
        meta: { hookName: 'useFetchChatList' },
    });
};

export const useFetchChatConversation = (receiverId, isAuthenticated) => {
    return useQuery({
        queryKey: ['chat-conversation', receiverId],
        queryFn: () => fetchChatConversationFn(receiverId),
        enabled: Boolean(isAuthenticated && receiverId),
        staleTime: 1000 * 15,
        gcTime: 1000 * 60 * 5,
        refetchOnWindowFocus: true,
        retry: 1,
        meta: { hookName: 'useFetchChatConversation' },
    });
};